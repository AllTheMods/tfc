...
const tfcNonFoodSeeds = ["pumpkin", "melon", "jute", "papyrus"]
const vanillaWoodTypes = ["acacia", "birch", "oak", "spruce"]
const tfcWoodTypes = [
  "ash",
  "aspen",
  "blackwood",
...
const dirtTypes = ["silt", "loam", "sandy_loam", "silty_loam"]
const tfcStoneTypes = [
  "granite",
  "diorite",
  "gabbro",
...
/**
 *
 * @param {Internal.RecipeEventJS} recipeEvent
 * @param {string} result
 * @param {number} count
...
const modifyShaped = (recipeEvent, result, count, pattern, ingredients) => {
  ...
  recipeEvent
    .shaped(Item.of(result, count), pattern, ingredients)
  ...
  for (let i = 1; i <= 8; i++) {
    recipesEvent
      .shapeless(
        ...
        ].concat(Array.from(new Array(i)).map((i) => input))
      )
      .id(`kubejs:${output.replace(`:`, `/`)}_${i}_manual_only`)
  }
  recipesEvent.recipes
    .createMixing(`${multiplier}x ${output}`, [Fluid.of(fluid, amount), input])
